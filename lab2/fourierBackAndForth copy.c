...
void main(int argc, char *argv[]){
    int row, column;
    float **data, **spectrum, **newSpectrum, **realValuesOnly, **butter, **newButter, **butteredSpectrum;
  ...
  fclose(input);                      // close input image after all data has been copied
    ...
    outputFile(outputFileName, newSpectrum);
    
    // make butterworth filter
    for (row=0; row<n; row++) {
        for (column=0; column<n; column++) {
            float center = sqrt(((column-128)*(column-128)) + ((row-128)*(row-128)));
            butter[row][column] = 1/(1+(sqrt(2)-1)*pow((center/D0),30));
        }
    }
